...
def fetch_weather_forecast(api_token: str, domain: str, province: str, amphoe: str, start_time: str):
    """
    ดึงข้อมูลพยากรณ์อากาศและบันทึกลงในไฟล์ CSV

    พารามิเตอร์:
    - api_token: โทเค็นในการเข้าถึง API
    - domain: พารามิเตอร์โดเมนสำหรับ API
    - province: ชื่อจังหวัดในภาษาไทย (เช่น 'นครปฐม')
    - amphoe: ชื่ออำเภอในภาษาไทย (เช่น 'สามพราน')
    """
...
if TMD_API_TOKEN:
    ...
    print(f"กำหนดเวลาเริ่มต้นการดึงข้อมูลวันที่ {start_time}")
    fetch_weather_forecast(TMD_API_TOKEN, domain="2", province="นครปฐม", amphoe="สามพราน", start_time=start_time)
else:
